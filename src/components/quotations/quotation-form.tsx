...
export function QuotationForm({ clients }: { clients: Client[] }) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      <Card>
        <CardHeader>
          <CardTitle className="text-foreground text-base font-semibold">Quotation Details</CardTitle>
        </CardHeader>
      ...
      <Card>
        ...
        <CardContent className="space-y-3 pt-0">
          {fields.map((field, index) => (
            <div
              key={field.id}
              className="grid grid-cols-1 gap-2 rounded-lg border border-border/70 p-3 sm:grid-cols-12 sm:items-end"
            >
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-foreground text-base font-semibold">Terms & Total</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4 pt-0">
          <div className="space-y-1.5">
            <Label>Terms & Conditions</Label>
            <Textarea placeholder="Valid for 30 days. 50% advance required..." {...register("terms")} />
          </div>
